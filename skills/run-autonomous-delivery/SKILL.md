...
8. **Review adversarially.** Review two axes independently:
   ...
   It is complete only after a real separate reviewer returns a result for the
   exact commit and the linked review receipt passes protected validation. If
   reviewer dispatch or provenance fails, preserve the tested work, keep the
   review and PR-ready gates incomplete, and report the blocker without
