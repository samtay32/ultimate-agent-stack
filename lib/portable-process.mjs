...
function windowsNodeShimInvocation(command, args, environment) {
  ...
  const configuredCli = typeof environment.npm_execpath === "string" && basename(environment.npm_execpath).toLowerCase() === cliName ? environment.npm_execpath : null;
  const cli = [
    configuredCli,
...
function npmInvocation(args, environment = process.env) {
  const configuredNpmCli = typeof environment.npm_execpath === "string" && basename(environment.npm_execpath).toLowerCase() === "npm-cli.js" ? environment.npm_execpath : null;
  const npmCli = [
    configuredNpmCli,
