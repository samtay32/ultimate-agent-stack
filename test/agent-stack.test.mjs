...
test("independent review evidence fails closed without worker provenance", () => {
  const fixture = temporaryProject();
  try {
    initializeGit(fixture.directory);
    createJavaScriptFixture(fixture.directory);
    ...
    commandConfigure(fixture.directory, {
      preset: "simple",
      reason: "Approved repository-only fixture configuration",
    });
    commandApproveChecks(
      fixture.directory,
      "Reviewed the detected JavaScript quality commands",
    );
    const git = (args) =>
      spawnSync("git", ["-C", fixture.directory, ...args], {
        encoding: "utf8",
        shell: false,
      });
    assert.equal(
      git(["config", "user.email", "review-test@example.invalid"]).status,
      0,
    );
    ...
    writeFileSync(
      ...
      [
        ...
        "Material open conflicts: NO",
        "",
        "Review the bounded status behavior.",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    assert.equal(
      git(["commit", "-m", "test: establish review baseline"]).status,
      0,
    );
    const baseRevision = git(["rev-parse", "HEAD"]).stdout.trim();
    writeFileSync(
      join(fixture.directory, "app.mjs"),
      "export const answer = 42;\n// reviewed change\n",
      "utf8",
    );
    assert.equal(git(["add", "app.mjs"]).status, 0);
    assert.equal(
      git(["commit", "-m", "fix: update reviewed behavior"]).status,
      0,
    );
    const reviewedRevision = git(["rev-parse", "HEAD"]).stdout.trim();

    const workItem = {
      id: "reviewed-change",
      title: "Review the bounded change",
      objective: "Require real independent review provenance.",
      status: "done",
      priority: "normal",
      acceptance_criteria: [
        "A separate reviewer passes standards and intent review.",
      ],
      scope: {
        paths: ["app.mjs"],
        out_of_scope: [],
      ...
      depends_on: [],
      evidence_refs: ["review-pre-pr"],
      external_refs: [],
      updated_at: null,
    };
    writeJson(join(fixture.directory, WORK_LEDGER_PATH), {
      schema_version: 1,
      updated_at: null,
      items: [workItem],
    });
      ...
      nodes: [
        ...
        },
        {
          ...
          source: {
            ...
            reference: `${REVIEW_RECEIPTS_PATH}/${"a".repeat(64)}.json`,
          },
          summary: "Standards and intent passed.",
        },
      ],
      edges: [
        {
    ...
    writeJson(join(fixture.directory, EVIDENCE_GRAPH_PATH), graph);
    writeFileSync(
      join(fixture.directory, PRE_PR_REVIEW_PATH),
      [
        ...
        "Material open conflicts: NO",
        "",
        "Unsupported primary-agent prose.",
        "",
      ].join("\n"),
      "utf8",
    );

    const missingEvidence = commandEvidenceValidate(fixture.directory);
    assert.equal(missingEvidence.ok, false);
    assert.match(
      missingEvidence.errors.join("\n"),
      /missing valid review receipt/,
    );
    assert.match(
      missingEvidence.errors.join("\n"),
      /APPROVED requires exactly one non-empty Review receipt field/,
    );
    const missingReceiptValidation =
      commandReceiptsValidate(fixture.directory);
    assert.equal(missingReceiptValidation.ok, false);
    assert.match(
      missingReceiptValidation.errors.join("\n"),
      /missing valid review receipt/,
    );
    const blockedVerify = commandVerify(fixture.directory);
    assert.equal(blockedVerify.ok, false);
    assert.match(
      blockedVerify.configuration_errors.join("\n"),
      /APPROVED requires exactly one non-empty Review receipt field/,
    );
    ...
    const receipt = {
      ...
      reviewer_id: "worker-123",
      base_revision: baseRevision,
      reviewed_revision: reviewedRevision,
      delivery_baseline:
        `.agent-stack/artifacts/DELIVERY.md@${baseRevision}`,
      standards_verdict: "passed",
      intent_verdict: "passed",
      reviewer_result_sha256: `sha256:${hashText("review result")}`,
      provenance_sha256: `sha256:${hashText("spawn and wait receipt")}`,
      read_only: true,
    ...
    assert.deepEqual(validateReviewReceipt(receipt), []);
    const receiptPath =
      `${REVIEW_RECEIPTS_PATH}/${receipt.receipt_id}.json`;
    writeJson(join(fixture.directory, receiptPath), receipt);
    graph.nodes[1].source.reference = receiptPath;
    writeJson(join(fixture.directory, EVIDENCE_GRAPH_PATH), graph);
    writeFileSync(
      join(fixture.directory, PRE_PR_REVIEW_PATH),
      [
        ...
        "## Scope",
        "",
        `- Assignment ID: ${receipt.assignment_id}`,
        `- Work item ID: ${receipt.work_item_id}`,
        `- Delivery baseline: ${receipt.delivery_baseline}`,
        `- Base revision: ${receipt.base_revision}`,
        `- Reviewed revision: ${receipt.reviewed_revision}`,
    ...
    const unsignedEvidence = commandEvidenceValidate(fixture.directory);
    assert.equal(unsignedEvidence.ok, false);
    assert.match(
      unsignedEvidence.errors.join("\n"),
      /unsigned and requires trusted outer attestation/,
    );
    const unsignedReceipts = commandReceiptsValidate(fixture.directory);
    assert.equal(unsignedReceipts.ok, false);
    assert.match(
      unsignedReceipts.errors.join("\n"),
      /unsigned and requires trusted outer attestation/,
    );
    const unsignedVerify = commandVerify(fixture.directory);
    assert.equal(unsignedVerify.ok, false);
    assert.match(
      unsignedVerify.configuration_errors.join("\n"),
      /unsigned and requires trusted outer attestation/,
    );
    const failedOuterAttestation = commandEvidenceValidate(
      fixture.directory,
      {
    ...
    assert.equal(failedOuterAttestation.ok, false);
    assert.match(
      failedOuterAttestation.errors.join("\n"),
      /trusted outer review attestation failed closed/,
    );
    assert.doesNotMatch(
      failedOuterAttestation.errors.join("\n"),
      /collector-secret-detail/,
    );
    const trustedReview = {
      validateTrustedAttestation(candidate, context) {
        return (
          candidate.receipt_id === receipt.receipt_id &&
          context.receipt_path === receiptPath &&
          context.node_id === "review-pre-pr"
        );
      },
    };
    const validEvidence = commandEvidenceValidate(
      fixture.directory,
      trustedReview,
    );
    assert.equal(validEvidence.ok, true, validEvidence.errors.join("\n"));
    const validReceipts = commandReceiptsValidate(
      fixture.directory,
      trustedReview,
    );
    assert.equal(validReceipts.ok, true, validReceipts.errors.join("\n"));
    assert.equal(validReceipts.receipt_count, 1);
    const postReviewVerify = commandVerify(
      fixture.directory,
      false,
      trustedReview,
    );
    ...
    assert.match(
      postReviewVerify.configuration_errors.join("\n"),
      /unsigned and requires trusted outer attestation/,
    );
  } finally {
    fixture.cleanup();
...
function currentReviewFixture() {
  ...
  commandConfigure(fixture.directory, {
    preset: "simple",
    reason: "Approved repository-only review fixture configuration",
  });
  commandApproveChecks(
    fixture.directory,
    "Reviewed the fixture JavaScript quality commands",
  );
  assert.equal(commandVerify(fixture.directory).ok, true);
  ...
  assert.equal(
    git(["config", "user.email", "review-fixture@example.invalid"]).status,
    0,
  );
  assert.equal(git(["config", "user.name", "Review Fixture"]).status, 0);
  assert.equal(git(["add", "."]).status, 0);
  assert.equal(
    git(["commit", "-m", "test: record draft installation"]).status,
    0,
  );
  const draftRevision = git(["rev-parse", "HEAD"]).stdout.trim();
  writeFileSync(
    join(
      fixture.directory,
      ".agent-stack",
    ...
    [
      ...
      "Material open conflicts: NO",
      "",
      "Correct the bounded status behavior.",
      "",
    ].join("\n"),
    "utf8",
  );
  assert.equal(git(["add", ".agent-stack/artifacts/DELIVERY.md"]).status, 0);
  assert.equal(git(["commit", "-m", "test: establish delivery baseline"]).status, 0);
  const baseRevision = git(["rev-parse", "HEAD"]).stdout.trim();
  writeFileSync(
    join(fixture.directory, "app.mjs"),
  ...
  assert.equal(git(["add", "app.mjs"]).status, 0);
  assert.equal(git(["commit", "-m", "fix: update status behavior"]).status, 0);
  const reviewedRevision = git(["rev-parse", "HEAD"]).stdout.trim();
  const workItem = {
    id: "reviewed-change",
    title: "Review the bounded change",
    objective: "Require current independently reviewed evidence.",
    status: "done",
    priority: "normal",
    acceptance_criteria: [
      "A trusted outer collector attests the separate review.",
    ],
    scope: {
      paths: ["app.mjs"],
      out_of_scope: [],
  ...
  const graph = {
    ...
    nodes: [
      ...
      },
      {
        ...
        summary: "Candidate review awaits trusted outer attestation.",
      },
    ],
    edges: [
      {
  ...
  context.writeCandidate = (overrides = {}) => {
    const receipt = {
      schema_version: 1,
      assignment_id: "review-pre-pr",
      work_item_id: "reviewed-change",
      ...
      mechanism: "native-subagent",
      harness: "test-harness",
      reviewer_id: "worker-456",
      base_revision: context.baseRevision,
      reviewed_revision: context.reviewedRevision,
      delivery_baseline:
        `.agent-stack/artifacts/DELIVERY.md@${context.baseRevision}`,
      standards_verdict: "passed",
      intent_verdict: "passed",
      reviewer_result_sha256: `sha256:${hashText("review result")}`,
      provenance_sha256: `sha256:${hashText("trusted collector event")}`,
      read_only: true,
      external_actions: false,
      started_at: "2026-07-30T15:00:00Z",
      completed_at: "2026-07-30T15:01:00Z",
  ...
  context.writeArtifact = (receipt, receiptPath, overrides = {}) => {
    const values = {
      status: "APPROVED",
      conflicts: "NO",
    ...
    writeFileSync(
      ...
      [
        ...
        `Material open conflicts: ${values.conflicts}`,
        "",
        "## Scope",
        "",
        `- Assignment ID: ${values.assignment_id}`,
        `- Work item ID: ${values.work_item_id}`,
        `- Delivery baseline: ${values.delivery_baseline}`,
        `- Base revision: ${values.base_revision}`,
        `- Reviewed revision: ${values.reviewed_revision}`,
        `- Review receipt: ${values.receipt_path}`,
        ...(overrides.extra_lines ?? []),
        "",
      ].join("\n"),
      "utf8",
    );
  };
  const current = context.writeCandidate();
  graph.nodes[1].source.reference = current.receiptPath;
  writeJson(join(fixture.directory, WORK_LEDGER_PATH), {
    schema_version: 1,
    updated_at: null,
    items: [workItem],
  ...
  writeJson(join(fixture.directory, EVIDENCE_GRAPH_PATH), graph);
  context.writeArtifact(current.receipt, current.receiptPath);
  context.current = current;
  context.trustedReview = {
    validateTrustedAttestation(candidate, attestationContext) {
      return (
        candidate.receipt_id === context.current.receipt.receipt_id &&
        attestationContext.receipt_path === context.current.receiptPath &&
        attestationContext.node_id ===
          context.current.receipt.evidence_node_id
      );
    },
  };
  return context;
...
test("historical review candidates do not poison a fresh current review", () => {
  const context = currentReviewFixture();
  try {
    const old = context.current;
    const failedHistorical = context.writeCandidate({
      assignment_id: "failed-historical-review",
      evidence_node_id: "failed-historical-review",
      base_revision: "a".repeat(40),
      reviewed_revision: "b".repeat(40),
      delivery_baseline:
        `.agent-stack/artifacts/DELIVERY.md@${"a".repeat(40)}`,
      standards_verdict: "failed",
      intent_verdict: "failed",
      result: "failed",
      reviewer_id: "unavailable-worker",
    });
    writeFileSync(
      join(context.fixture.directory, "app.mjs"),
      "export const answer = 43;\n// repair after first review\n",
      "utf8",
    );
    assert.equal(
      context.git(["add", "app.mjs", old.receiptPath, failedHistorical.receiptPath])
        .status,
      0,
    );
    assert.equal(
      context.git(["commit", "-m", "fix: address first review"]).status,
      0,
    );
    context.reviewedRevision = context.git(["rev-parse", "HEAD"]).stdout.trim();
    context.graph.nodes[1].state = "superseded";
    const currentNode = {
      id: "review-pre-pr-current",
      kind: "review",
      label: "Fresh independent pre-PR review",
      state: "verified",
      source: {
        provider: "review-receipt",
        reference: "",
      },
      summary: "Fresh candidate review.",
    };
    context.graph.nodes.push(currentNode);
    context.graph.edges.push({
      from: currentNode.id,
      to: context.workItem.id,
      relation: "reviews",
    });
    context.workItem.evidence_refs = [currentNode.id];
    const fresh = context.writeCandidate({
      assignment_id: currentNode.id,
      evidence_node_id: currentNode.id,
      reviewed_revision: context.reviewedRevision,
      reviewer_id: "worker-789",
      reviewer_result_sha256: `sha256:${hashText("fresh review result")}`,
      provenance_sha256: `sha256:${hashText("fresh collector event")}`,
    });
    currentNode.source.reference = fresh.receiptPath;
    context.current = fresh;
    writeJson(join(context.fixture.directory, WORK_LEDGER_PATH), {
      schema_version: 1,
      updated_at: null,
      items: [context.workItem],
    });
    writeJson(
      join(context.fixture.directory, EVIDENCE_GRAPH_PATH),
      context.graph,
    );
    context.writeArtifact(fresh.receipt, fresh.receiptPath);

    const result = commandEvidenceValidate(
      context.fixture.directory,
      context.trustedReview,
    );
    assert.equal(result.ok, true, result.errors.join("\n"));
    assert.equal(existsSync(join(context.fixture.directory, old.receiptPath)), true);

    const lateHistorical = context.writeCandidate({
      assignment_id: "late-historical-review",
      evidence_node_id: "late-historical-review",
      standards_verdict: "failed",
      intent_verdict: "failed",
      result: "failed",
      reviewer_id: "unavailable-worker",
    });
    const lateResult = commandEvidenceValidate(
      context.fixture.directory,
      context.trustedReview,
    );
    assert.match(
      lateResult.errors.join("\n"),
      new RegExp(
        `product paths changed after review: ${lateHistorical.receiptPath.replaceAll(".", "\\.")}`,
      ),
    );
  } finally {
...
test("local review linkage and approved artifact fields fail closed", () => {
  const context = currentReviewFixture();
  try {
    context.graph.edges = [];
    writeJson(
    ...
    let result = commandEvidenceValidate(
      context.fixture.directory,
      context.trustedReview,
    );
    assert.match(result.errors.join("\n"), /requires an exact reviews edge/);
    ...
    context.workItem.evidence_refs = [];
    writeJson(join(context.fixture.directory, WORK_LEDGER_PATH), {
      schema_version: 1,
      updated_at: null,
      items: [context.workItem],
    ...
    result = commandEvidenceValidate(
      context.fixture.directory,
      context.trustedReview,
    );
    assert.match(result.errors.join("\n"), /through evidence_refs/);

    context.workItem.evidence_refs = ["review-pre-pr"];
    writeJson(join(context.fixture.directory, WORK_LEDGER_PATH), {
      schema_version: 1,
      updated_at: null,
      items: [context.workItem],
    ...
    context.writeArtifact(
      context.current.receipt,
      context.current.receiptPath,
    ...
    assert.match(
      result.errors.join("\n"),
      /work_item_id does not match the current review candidate/,
    );

    context.writeArtifact(
      context.current.receipt,
      context.current.receiptPath,
      { extra_lines: ["Status: APPROVED"] },
    );
    result = commandEvidenceValidate(
      context.fixture.directory,
      context.trustedReview,
    ...
    assert.match(
      result.errors.join("\n"),
      /cannot be APPROVED while material conflicts remain/,
    );
    ...
    writeFileSync(
      join(context.fixture.directory, ".agent-stack", "unreviewed-note.txt"),
      "not evidence packaging\n",
      "utf8",
    );
    result = commandEvidenceValidate(
      context.fixture.directory,
    ...
    assert.match(
      result.errors.join("\n"),
      /product paths changed after review: \.agent-stack\/unreviewed-note\.txt/,
    );

    const wrongBaseline = {
      ...context.current.receipt,
      delivery_baseline:
        `.agent-stack/artifacts/DELIVERY.md@${context.reviewedRevision}`,
    };
    delete wrongBaseline.receipt_id;
    wrongBaseline.receipt_id = reviewReceiptId(wrongBaseline);
    assert.match(
      validateReviewReceipt(wrongBaseline).join("\n"),
      /delivery_baseline must equal .*<base_revision>/,
    );
  } finally {
    context.fixture.cleanup();
  }
...
test("legacy external review nodes remain valid beside a local review", () => {
  const context = currentReviewFixture();
  try {
    context.graph.nodes.push({
      id: "external-provider-review",
      kind: "review",
      label: "Existing provider review",
      state: "verified",
      source: {
        provider: "coderabbit",
        reference: "pull-request/123/review",
      },
      summary: "Legacy external review evidence.",
    });
    writeJson(
      join(context.fixture.directory, EVIDENCE_GRAPH_PATH),
      context.graph,
...
test("current review base must point to an approved delivery baseline", () => {
  const context = currentReviewFixture();
  try {
    const invalid = context.writeCandidate({
      base_revision: context.draftRevision,
      delivery_baseline:
        `.agent-stack/artifacts/DELIVERY.md@${context.draftRevision}`,
      reviewer_result_sha256: `sha256:${hashText("invalid baseline review")}`,
    });
    context.current = invalid;
    context.graph.nodes[1].source.reference = invalid.receiptPath;
    writeJson(
      join(context.fixture.directory, EVIDENCE_GRAPH_PATH),
      context.graph,
    );
    context.writeArtifact(invalid.receipt, invalid.receiptPath);
    const result = commandEvidenceValidate(
      context.fixture.directory,
      context.trustedReview,
    );
    assert.match(
      result.errors.join("\n"),
      /delivery baseline must be an approved conflict-free DELIVERY\.md/,
    );
  } finally {
    context.fixture.cleanup();
  }
...
test(
  "review Git inspection ignores hostile environment and fsmonitor configuration",
  { skip: platform() === "win32" },
  () => {
    const context = currentReviewFixture();
    const outside = temporaryProject();
    ...
    try {
      ...
      assert.equal(
        context.git(["config", "core.fsmonitor", monitor]).status,
        0,
      );
      assert.equal(
        context.git(["status", "--porcelain=v1"]).status,
        0,
      );
      ...
      for (const [name, value] of Object.entries({
        ...
        GIT_WORK_TREE: outside.directory,
      })) {
...
test("local review validation ignores repository replacement refs", () => {
  const context = currentReviewFixture();
  try {
    const actualTree = context.git([
      "--no-replace-objects",
      "rev-parse",
      "HEAD^{tree}",
    ]).stdout.trim();
    ...
    assert.equal(
      context.git(["rev-parse", "HEAD^{tree}"]).stdout.trim(),
      replacementTree,
      "fixture must prove the repository replacement ref is active",
    );
    assert.equal(
      context.git([
        "--no-replace-objects",
...
test(
  "review validation rejects symlinked repository evidence inputs and parents",
  { skip: platform() === "win32" },
  () => {
    const receiptContext = currentReviewFixture();
    try {
      const directory = join(
        receiptContext.fixture.directory,
        REVIEW_RECEIPTS_PATH,
      );
      const actual = join(
        receiptContext.fixture.directory,
      ...
      const result = commandEvidenceValidate(
        receiptContext.fixture.directory,
        receiptContext.trustedReview,
      );
      assert.match(
        result.errors.join("\n"),
        /review receipts directory crosses a symlinked path component/,
      );
    } finally {
      receiptContext.fixture.cleanup();
    }

    const artifactContext = currentReviewFixture();
    try {
      const directory = join(
        artifactContext.fixture.directory,
        ".agent-stack",
        "artifacts",
      );
      const actual = join(
      ...
      const result = commandEvidenceValidate(
        artifactContext.fixture.directory,
        artifactContext.trustedReview,
      );
      assert.match(
        result.errors.join("\n"),
        /pre-PR review artifact crosses a symlinked path component/,
      );
    } finally {
      artifactContext.fixture.cleanup();
    }

    const contractContext = currentReviewFixture();
    try {
      const ledger = join(
        contractContext.fixture.directory,
        WORK_LEDGER_PATH,
      );
      ...
      const actualGraph = join(
        contractContext.fixture.directory,
        ".agent-stack",
        "actual-evidence-graph.json",
      );
      renameSync(graph, actualGraph);
      symlinkSync(actualGraph, graph, "file");

      const workResult = commandWorkValidate(
        contractContext.fixture.directory,
      );
      assert.match(
        workResult.errors.join("\n"),
        /work ledger crosses a symlinked path component/,
      );
      const evidenceResult = commandEvidenceValidate(
        contractContext.fixture.directory,
        contractContext.trustedReview,
      );
      assert.equal(evidenceResult.work_ledger.ok, false);
      assert.match(
        evidenceResult.errors.join("\n"),
        /evidence graph crosses a symlinked path component/,
      );
    } finally {
      contractContext.fixture.cleanup();
    }

    const parentContext = currentReviewFixture();
    try {
      const directory = join(parentContext.fixture.directory, ".agent-stack");
      const actual = join(
        parentContext.fixture.directory,
        ".actual-agent-stack",
      );
      renameSync(directory, actual);
      symlinkSync(actual, directory, "dir");

      const workResult = commandWorkValidate(parentContext.fixture.directory);
      assert.match(
        workResult.errors.join("\n"),
        /work ledger crosses a symlinked path component/,
      );
      ...
      parentContext.fixture.cleanup();
    }
  },
);

test("upgrade creates review candidate controls missing from legacy installs", () => {
  const fixture = temporaryProject();
  try {
    initializeGit(fixture.directory);
    installOrUpgrade(fixture.directory, { mode: "init" });
    ...
    const legacyMissing = [
      PRE_PR_REVIEW_PATH,
      ".agent-stack/contracts/review-receipt.schema.json",
      `${REVIEW_RECEIPTS_PATH}/.gitkeep`,
    ];
    for (const path of legacyMissing) {
      delete installation.managed_files[path];
      rmSync(join(fixture.directory, path), { force: true });
    }
    rmSync(join(fixture.directory, REVIEW_RECEIPTS_PATH), {
      recursive: true,
      force: true,
    });
    writeJson(installationFile, installation);

    const upgraded = installOrUpgrade(fixture.directory, {
      mode: "upgrade",
    });
    for (const path of legacyMissing) {
      assert.equal(
        upgraded.outcomes.find((outcome) => outcome.path === path)?.status,
        "created",
      );
      assert.equal(existsSync(join(fixture.directory, path)), true);
    }
  } finally {
    fixture.cleanup();
  }
